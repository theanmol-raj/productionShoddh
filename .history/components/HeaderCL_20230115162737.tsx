...
function HeaderCL() {
  ...
  useEffect(()=>{
    if(address){
      dispatch(action.login(address))
    }else{
      dispatch(action.logout(address))
    }
  },[address])
      ...
      <div className="  p-3  mx-auto max-w-7xl  ">
        ...
        <div className=" flex items-center space-x-4">
          ...
          <div className=" space-x-4 text-sm">
            {/* logins */}
            {address ? <Dropdown user={address} /> : <button onClick={()=>{
                connect()
            }} className="  px-4 text-white bg-gradient-to-l from-blue-400 font-medium rounded-full text-sm hover:to-yellow-400 hover:from-yellow-400 hover:text-black to-purple-500 py-1">Connect Wallet</button>
            }
          </div>
