...
export default const RichTextComponents = {
    types : {
    ...
    blockquote : ({children} :any) => <blockquote className='pl-5 py-5 my-5 border-l-4 border-l-[#f7ab0a]'>{children}</blockquote>
    }
    ,
    marks :  {
        link : ({children ,value} : any) => {
            const rel = !value.href.startsWith("/") ? "noRefError NoOpener" : undefined ;
            return (
                <Link href={value.href} rel={rel} className="text-green-700"> 
                    {children}
                </Link>
            )
