...
function AuthorCard({data}:Props) {
  ...
  const handleClick = (e :any) => {
    e.preventDefault()
    router.push(`/author/${data.slug.current}`)
  }
    ...
    <div className='rounded-lg border '>
        ...
        <div className='pt-20 px-6 pb-8'>
          <h1 className=' text-center text-xl font-serif font-bold'>{data?.name}</h1>
            ...
            <div className=''>
            <button onClick={handleClick} className=' text-lg border-black border font-bold w-full py-2 my-2 mt-8'>Visit Profile</button> 
