...
const query = groq`
{
    'ds' : *[_type=="post" && "ds" in categories[]->title && !EP ][0..4]{
      _id,
  title,
  slug,
  mainImage,
  ...
  'ml': *[_type=="post" && "ml" in categories[]->title && !EP ][0...3]{
    _id,
title,
slug,
mainImage,
...
'nlp' : *[_type=="post" && "nlp" in categories[]->title && !EP ][0...3]{
  _id,
title,
slug,
mainImage,
...
'cv' : *[_type=="post" && "cv" in categories[]->title && !EP ][0..3]{
  _id,
title,
slug,
mainImage,
...
async function Page() {
  const [parentQ ,setParentQ] = useState<any>(null) ;
  useEffect(() => {
    async function getData(){
      await client.fetch(query).then(data=>setParentQ(data)).catch(er => console.log(er))
    }
...
// console.log(parentQ)
  if (!parentQ) return <h1 className=' h-screen'>Loading </h1>
  return (
    <div className=" pt-40">
      {/* <Navbar /> */}
      <div className=" max-w-7xl mx-auto px-4 md:px-0">
        <Carousel data={[parentQ?.EP[0] , parentQ?.EP[1]]} />
